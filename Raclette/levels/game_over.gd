...
export(String, FILE) var retry_path: String
onready var _retry_scene: = load(retry_path)
...
func _on_Retry_pressed() -> void:
	get_tree().change_scene_to(_retry_scene)
...
func _get_configuration_warning() -> String:
	if not retry_path:
		return ErrorMessages.get_message("The retry scene path", ErrorMessages.Warning.SHOULD_NOT_BE_EMPTY)
